<!-- meta: Known constraints, performance ceilings, known bugs, unresolved items -->
...
- `.specback/drafts/08-compatibility.md` (lines 206-211)
...
### 13.1.6 Question Bank の自動マージは限定的
...
### 13.2.1 Depth モードのファイル数カウント基準

`comprehensive` モード自動選択の閾値「200ファイル以下」におけるファイル数のカウント基準は以下の通り。

- **カウント対象**: 偵察スキャンで発見された全ファイル（`.git`, `node_modules`, `vendor`, `__pycache__` 等のノイズディレクトリを除外した後）。ソースコード、テストファイル、設定ファイル、ドキュメント等を含む。
- **カウント対象外**: `LANG_BY_EXT` に登録された拡張子のみに限定しない（全ファイルが対象）。
- **根拠**: 200ファイルという閾値は spec 生成にかかるおおよその時間を推定するヒューリスティックである。設定ファイルのみが大量にあるリポジトリでも絶対的なスキャン・処理時間は増加するため、総ファイル数で判定する。
- **記録場所**: `state.json` の `total_files` フィールドに記録され、`recon-report.md` の先頭にも記載される [REF: phase-1-recon.md:58-70]。

この閾値は specback の処理時間を推定するためのヒューリスティックであり、精密な制御が必要な場合はユーザーが明示的に depth_mode を指定することで上書きできる。

### 13.2.2 `abandoned` 判定の SLA
...
### 13.2.3 Phase 7 と Phase 5 の相互作用

ドリフト検出 (Phase 7) 後に自動的に Phase 5 の対話プロセスを再実行するか否かの仕様は、v1.x では「**レポートのみ**」と確定した。

Phase 7 は以下の流れで完了する:

1. `detect-drift.py` を実行し、`drift-report.md` を生成する
2. ユーザーに「ドリフトレポートが生成されました。REF 行番号を自動修正しますか？（Phase 7b）」と確認する
3. ユーザーが Yes を選択した場合のみ Phase 7b に進む（REF 行番号の自動修正）
4. 必要に応じて Phase 7c（ChangeSpec）を実行し、変更仕様書を生成する

ドリフトによって spec の内容自体を更新する必要がある場合は、ユーザーが `drift-report.md` を確認した上で、手動で spec を編集する。Phase 5 への自動遷移は v2.0.0 以降のロードマップ項目とする。

**決定理由**:

- Phase 5 はユーザーとの対話が必須であり、CI や定期実行などユーザー不在のタイミングでは実行できない
- Phase 7b（REF 自動修正）と Phase 7c（ChangeSpec）でほとんどのドリフト対応はカバーできる
- 仕様内容の実質的な更新が必要なケースでは、人間の判断が不可欠である

### 13.2.4 Question Bank 自動マージのアルゴリズム詳細
...
### 13.2.5 テンプレート選定判定ツリー

テンプレート選定の判定ツリーは `references/template-catalog.md` の "Decision tree (Claude's recommendation logic)" に定義されている。この判定ツリーは AI エージェントが Phase 1 で解釈・実行するプロトコルであり、決定論的なコード実装を持たない。これは以下の理由から意図的な設計判断である。

- **定性判断への依存**: 「パッケージマニフェストに main が定義されているか」「views/ ディレクトリが存在するか」などの判断はコードベースの文脈理解を必要とし、純粋なコードロジックでは扱いにくい。
- **複合プロジェクト対応**: 複数のテンプレートにまたがるプロジェクトではユーザーとの対話による調整が前提であり、決定論的な選定は適さない。
- **ユーザー選択権の確保**: AI エージェントの推奨は参考情報であり、最終的なテンプレート選択はユーザーに委ねられている。

この設計判断により、判定ツリーの更新は `template-catalog.md` の編集のみで完結し、コード変更を伴わない。テンプレート選定ロジックに変更が必要な場合は `template-catalog.md` の Decision tree セクションを編集すること。
...
Dual-consumer（複数エージェント間での `.specback/` 共有）は以下の理由から推奨しない [REF: 99-unresolved.md:Q-010]。

- **スキル読み込みパスの非互換性**: 各エージェント（Claude Code, OpenCode, Codex CLI 等）は独自のスキルディレクトリパス解決機構を持ち、他エージェントが生成した `state.json` のパス情報を正しく解決できない。
- **Task ツール API の差異**: Phase 3 の並列ディスパッチに使用する Task ツールのインターフェースがエージェントごとに異なる。同一の `wbs.json` でもディスパッチ結果が異なる可能性がある。
- **状態管理の不整合**: `state.json` は単一エージェントのセッション進行を前提として設計されている。エージェントを切り替えると `phase_progress` や `current_draft` の解釈がずれる。
- **意図しない上書きリスク**: エージェント A が生成したドラフトをエージェント B が上書きする可能性がある。specback には排他制御機構がない。

やむを得ず Dual-consumer 運用を行う場合は、以下の最小ガイドラインに従うこと:

1. 必ず片方のエージェントでの作業が完全に終了してから切り替える（同時操作禁止）
2. 切り替え前に `state.json` を手動で確認し、期待通りの状態であることを検証する
3. テンプレート選定やゴール設定は最初のエージェントで完了させ、後続エージェントでは変更しない
4. 両エージェントで同一バージョンの specback を使用する
