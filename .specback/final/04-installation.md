...
### 4.1.2 Windows インストーラ (`install.ps1`)
...
### 4.1.3 dry-run モード (`--dry-run` / `-DryRun`)
...
### 4.1.4 オプション依存関係のインストール (`--install-deps`)
...
### 4.3.2 `.opencode/skills/` と `skills/` の二重配置
...
### 4.7.1 `install.sh` が `skills/specback/` を見つけられない
...
### 4.7.2 エージェントが specback スキルを認識しない
...
### 4.7.3 gitleaks が pre-commit で失敗する
...
### 4.7.4 tree-sitter のインストールに失敗する
...
インストールに関する詳細は README.md のインストール節を参照。
