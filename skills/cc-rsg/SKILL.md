...
## 7-phase state machine overview

The skill is implemented as a 7-phase state machine. Progress is tracked in `.cc-rsg/state.json` and is pause-and-resume safe.
...
| 7 | Drift Detection | `drift-report.md` (code-change → spec impact) |
| 7b | REF Auto-Fix | auto-corrected REF line numbers |
| 7c | ChangeSpec | `change-spec.md` (human-readable change specification) |
...
- `.cc-rsg/` directory with `source-map.json` and `trace.json` (generated by Phase 4 / build-trace.py)
...
1. **Mode detection** (auto)
   ...
   - `.cc-rsg/source-hashes.json` exists → **hash mode**

2. **Run detect-drift.py**
   ```bash
   python .cc-rsg/skill/scripts/detect-drift.py \
     --cc-rsg-dir .cc-rsg \
     --output-dir {output_dir}
   ```
...
python .cc-rsg/skill/scripts/detect-drift.py --cc-rsg-dir .cc-rsg

# Explicit git mode
python .cc-rsg/skill/scripts/detect-drift.py --cc-rsg-dir .cc-rsg --mode git --base v1.0

# Hash mode (non-Git projects)
python .cc-rsg/skill/scripts/detect-drift.py --cc-rsg-dir .cc-rsg --mode hash

# Pipe diff from CI
git diff --name-status main...HEAD | python .cc-rsg/skill/scripts/detect-drift.py --diff -

# JSON output
python .cc-rsg/skill/scripts/detect-drift.py --cc-rsg-dir .cc-rsg --json
```
...
2. **Run change-spec.py** (mechanical extraction):
   ```bash
   python .cc-rsg/skill/scripts/change-spec.py \
     --cc-rsg-dir .cc-rsg \
     --output-dir {output_dir}
   ```
...
```bash
# Git mode (default)
python .cc-rsg/skill/scripts/change-spec.py --cc-rsg-dir .cc-rsg

# Hash mode
python .cc-rsg/skill/scripts/change-spec.py --cc-rsg-dir .cc-rsg --mode hash

# Pipe diff
git diff -U5 main...HEAD | python .cc-rsg/skill/scripts/change-spec.py --diff -
```
