...
-----
    # Git mode (default)
    python change-spec.py --cc-rsg-dir .cc-rsg
    python change-spec.py --cc-rsg-dir .cc-rsg --base HEAD
    python change-spec.py --cc-rsg-dir .cc-rsg --diff < git-diff-output.txt

    # Hash mode (non-Git projects)
    python change-spec.py --cc-rsg-dir .cc-rsg --mode hash

    # Explicit mode
    python change-spec.py --cc-rsg-dir .cc-rsg --mode git
...
Output
------
    .cc-rsg/change-spec.json   (structured facts — consumed by AI agent)
"""
...
def resolve_mode(args_mode: str | None, cc_rsg_path: Path) -> str:
...
def build_change_spec(
    ...
    source_map = load_source_map(cc_rsg_path / "source-map.json")
    trace = load_trace(cc_rsg_path / "trace.json")
    target_root = source_map.get("target_root", cc_rsg_path.parent)
    ...
    if mode == MODE_GIT:
        if diff_text is not None:
            ...
            resolved_base = resolve_base(base, cc_rsg_path)
            name_status_changes = run_git_diff_name_status(
                resolved_base, cwd=str(cc_rsg_path.parent),
            )
            unified_diff_text = run_git_unified_diff(
                resolved_base, cwd=str(cc_rsg_path.parent),
            )
    elif mode == MODE_HASH:
        source_hashes = load_source_hashes(cc_rsg_path / "source-hashes.json")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="cc-rsg ChangeSpec: mechanically extract structured change data",
    )
    parser.add_argument(
        "--cc-rsg-dir",
        default=".cc-rsg",
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output path for change-spec.json (default: <cc-rsg-dir>/change-spec.json)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = build_change_spec(
        cc_rsg_path=cc_rsg_path,
        mode=mode,
        base=base,
        diff_text=args.diff,
    )

    output_path = args.output or str(cc_rsg_path / "change-spec.json")
