...
-----
    ...
    python fix-refs.py --cc-rsg-dir .cc-rsg
    python fix-refs.py --cc-rsg-dir .cc-rsg --base HEAD
    python fix-refs.py --cc-rsg-dir .cc-rsg --diff < git-diff-output.txt

    # Apply corrections to spec files
    python fix-refs.py --cc-rsg-dir .cc-rsg --apply

    # Strict mode: exit 1 if any orphaned REFs remain
    python fix-refs.py --cc-rsg-dir .cc-rsg --check
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="cc-rsg Phase 7b: auto-correct [REF: ...] line numbers",
    )
    p.add_argument("--cc-rsg-dir", default=".cc-rsg",
                    help="Path to .cc-rsg/ state directory")
    ...
    if not cc_rsg_path.is_dir():
        print(f"ERROR: {args.cc_rsg_dir} is not a directory.", file=sys.stderr)
        return 2

    # --output-dir defaults to --cc-rsg-dir if not specified
    ...
    if args.diff is not None:
        ...
        print(f"fix-refs.py: diffing against --base {base[:12]}",
              file=sys.stderr)
        diff_text = get_git_diff(base, cwd=str(cc_rsg_path.parent))
