#!/usr/bin/env python3
"""
cc-rsg build-knowledge-graph.py
...
Usage:
    python build-knowledge-graph.py \
        ...
        --trace .cc-rsg/trace.json \
        --inventory .cc-rsg/inventory.json \
        --output .cc-rsg/knowledge-graph.jsonld
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--source-map", type=Path, default=".cc-rsg/source-map.json",
        help="Path to source-map.json (default: .cc-rsg/source-map.json)",
    )
    p.add_argument(
        "--trace", type=Path, default=".cc-rsg/trace.json",
        help="Path to trace.json (default: .cc-rsg/trace.json)",
    )
    p.add_argument(
        "--inventory", type=Path, default=".cc-rsg/inventory.json",
        help="Path to inventory.json (default: .cc-rsg/inventory.json)",
    )
    p.add_argument(
        "--questions", type=Path, default=None,
        help="Path to questions.json (optional; auto-detected from --cc-rsg-dir if omitted)",
    )
    p.add_argument(
        "--cc-rsg-dir", type=Path, default=None,
        help="Path to .cc-rsg/ directory (used to auto-discover questions.json)",
    )
    p.add_argument(
        "--output", type=Path, default=".cc-rsg/knowledge-graph.jsonld",
        help="Path to output JSON-LD file (default: .cc-rsg/knowledge-graph.jsonld)",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Auto-discover questions path from --cc-rsg-dir if --questions not given
    questions_path: Path | None = args.questions
    if questions_path is None and args.cc_rsg_dir is not None:
        questions_path = args.cc_rsg_dir / "questions.json"
    elif questions_path is None:
        # Try default location
        questions_path = Path(".cc-rsg/questions.json")
