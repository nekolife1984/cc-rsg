...
This is the core script for cc-rsg Phase 7 (Drift Detection).

Usage
-----
    # Git mode (default when Git repo available)
    python detect-drift.py --cc-rsg-dir .cc-rsg
    python detect-drift.py --cc-rsg-dir .cc-rsg --base v1.0
    python detect-drift.py --cc-rsg-dir .cc-rsg --diff < git-diff-output.txt

    # Hash mode (for non-Git projects)
    python detect-drift.py --cc-rsg-dir .cc-rsg --mode hash

    # Explicit mode selection
    python detect-drift.py --cc-rsg-dir .cc-rsg --mode git
    python detect-drift.py --cc-rsg-dir .cc-rsg --mode auto
...
def generate_markdown(
    ...
    if total_affected == 0:
        ...
        lines.append("   ```bash")
        lines.append("   python {skill_dir}/scripts/build-trace.py --cc-rsg-dir .cc-rsg --target-dir-for-required final")
        lines.append("   python {skill_dir}/scripts/build-traceability.py --cc-rsg-dir .cc-rsg --output-dir final")
        lines.append("   ```")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="cc-rsg drift detection: identify spec sections affected by code changes",
    )
    parser.add_argument(
        "--cc-rsg-dir",
        default=".cc-rsg",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory for drift reports (default: same as --cc-rsg-dir)",
    )
    parser.add_argument(
        "--mode",
        default=None,
...
def resolve_base(args_base: str | None, cc_rsg_path: Path) -> str:
    ...
    state = load_state(cc_rsg_path / "state.json")
...
def resolve_mode(
    ...
    has_git = (cc_rsg_path.parent / ".git").is_dir()
    state = load_state(cc_rsg_path / "state.json")
    has_generated_commit = bool(state and state.get("generated_at_commit"))
    has_source_hashes = (cc_rsg_path / "source-hashes.json").exists()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not cc_rsg_path.is_dir():
        print(
            ...
            "Run from the target project root or pass --cc-rsg-dir.",
            file=sys.stderr,
        )
        return 2

    # Resolve output directory
    output_dir = Path(args.output_dir) if args.output_dir is not None else cc_rsg_path
    output_dir.mkdir(parents=True, exist_ok=True)

    # -- Resolve detection mode --
    mode = resolve_mode(args.mode, cc_rsg_path)
    print_mode_info(mode)

    # -- Load shared artifacts --
    source_map_path = cc_rsg_path / "source-map.json"
    trace_path = cc_rsg_path / "trace.json"
    ...
    if mode == MODE_HASH:
        ...
        source_hashes = load_source_hashes(
            cc_rsg_path / "source-hashes.json",
        )
        ...
        # Git mode
        base = resolve_base(args.base, cc_rsg_path)
        print_base_info(base)
        changes = run_git_diff_name_status(base, cwd=str(cc_rsg_path.parent))
    ...
    # -- Generate reports --
    md = generate_markdown(result, base, len(changes), args.cc_rsg_dir)
    output_path = args.output or str(output_dir / "drift-report.md")
