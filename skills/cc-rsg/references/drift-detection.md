...
Phase 7 (Drift Detection) adds a **reverse traceability check** to cc-rsg. While cc-rsg's main direction is **code → spec** (reverse engineering), Phase 7 answers: "*After source code changes, which spec sections need attention?*"

This is the **foundation** for future incremental update features (Phase 7b REF Auto-Fix, Phase 7c ChangeSpec).

## Problem

After cc-rsg generates a spec (Phase 6), the codebase keeps evolving. Without drift detection:
...
| `.cc-rsg/` doesn't exist | Exit code 2 |
...
3. Scan spec files in `.cc-rsg/drafts/` or `{output_dir}/` (default: `.cc-rsg/final/`) for `[REF: path:line]` markers
...
```bash
# Dry-run (default)
python .cc-rsg/skill/scripts/fix-refs.py --cc-rsg-dir .cc-rsg

# Apply corrections
python .cc-rsg/skill/scripts/fix-refs.py --cc-rsg-dir .cc-rsg --apply

# CI check: exit 1 if any orphaned REFs remain
python .cc-rsg/skill/scripts/fix-refs.py --cc-rsg-dir .cc-rsg --check

# Pipe diff from CI
git diff -U0 main...HEAD | python .cc-rsg/skill/scripts/fix-refs.py --diff - --check
```

### Safety

- **Default is dry-run**: files are not modified until `--apply` is passed
- **Backups**: original files saved to `.cc-rsg/backups/<file>.bak` before modification
